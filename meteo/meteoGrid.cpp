...
void Crit3DMeteoGrid::aggregateHourlyInDaily(meteoVariable myVar, Crit3DDate dateIni, Crit3DDate dateFin)
{
    /*
    Dim myRow As Long, myCol As Long
    Dim ora As Byte, quadr As Byte
    Dim iday As Integer
    Dim cont As Single, dirPre As Single
    Dim temp As Single, Umid As Single, prech As Single, radh As Single, etph As Single
    Dim IntVento As Single, dirVento As Single
    Dim sum As Single
    Dim ValMin As Single, valMed As Single, ValMax As Single
    Dim direzione(8) As Byte, intensita(8) As Single
    Dim myETP As New Etp
    Dim myEtpAlgorithm As Byte

    For myRow = 0 To MeteoGrid.structure.header.nrRows - 1
        For myCol = 0 To MeteoGrid.structure.header.nrCols - 1
            If MeteoGrid.Point(myRow, myCol).active Then
                With MeteoGrid.Point(myRow, myCol).values_D

                    'TEMPERATURE
                    If computeTemp Then
                        ValMin = NO_DATA: valMed = NO_DATA: ValMax = NO_DATA
                        sum = 0: cont = 0

                        For ora = 1 To 24
                            temp = MeteoGrid.Point(myRow, myCol).values_H.Tavg_h(ora)
                            If temp <> NO_DATA Then
                                ValMin = IIf(ValMin = NO_DATA, temp, min(temp, ValMin))
                                ValMax = IIf(ValMax = NO_DATA, temp, max(temp, ValMax))
                                sum = sum + temp
                                cont = cont + 1
                            End If
                        Next ora

                        If cont > 0 Then valMed = sum / cont
                        If computeDailyExtremes Then .Tmax = ValMax
                        If computeDailyExtremes Then .Tmin = ValMin
                        .Tavg = valMed

                        'ETP HARGREAVES
                        If computeEt0Hargreaves Then
                            If .Tmin <> Definitions.NO_DATA And .Tmax <> NO_DATA Then
                                .ET0_HS = myETP.Hargreaves(Environment.TransSamaniCoefficient, MeteoGrid.Point(myRow, myCol).latitude, TimeUtility.DateToDOY(.date), .Tmax, .Tmin)
                            Else
                                .ET0_HS = Definitions.NO_DATA
                            End If
                        End If
                    End If
                    ...
                    If computeRH Then
                        ...
                        For ora = 1 To 24
                            Umid = MeteoGrid.Point(myRow, myCol).values_H.RHavg_h(ora)
                            If Umid <> NO_DATA Then
                                ValMin = IIf(ValMin = NO_DATA, Umid, min(Umid, ValMin))
                                ValMax = IIf(ValMax = NO_DATA, Umid, max(Umid, ValMax))
                                sum = sum + Umid
                                cont = cont + 1
                            End If
                        Next ora
                    ...
                    'PRECIPITAZIONI
                    If computePrec And computeDailyExtremes Then
                        sum = 0: cont = 0

                        For ora = 1 To 24
                            prech = MeteoGrid.Point(myRow, myCol).values_H.prec_H(ora)
                            If prech <> NO_DATA Then
                                sum = sum + prech
                                cont = cont + 1
                            End If
                        Next ora

                        .prec = IIf(cont > 0, sum, Definitions.NO_DATA)
                    End If

                    'VENTO
                    'media scalare dell'intensita'
                    'direzione prevalente a semi-quadranti
                    'GA 03-02-2006 aggiunto vento massimo
                    If computeWind Then

                        sum = 0: cont = 0: ValMax = NO_DATA
                        ...
                        For ora = 1 To 24
                            IntVento = MeteoGrid.Point(myRow, myCol).values_H.vmed_int_h(ora)
                            dirVento = MeteoGrid.Point(myRow, myCol).values_H.vmed_dir_h(ora)

                            If IntVento <> NO_DATA Then
                                sum = sum + IntVento
                                ValMax = IIf(ValMax = NO_DATA, IntVento, max(IntVento, ValMax))
                                cont = cont + 1
                            End If

                            If dirVento <> NO_DATA And IntVento <> NO_DATA And dirVento >= 0 And IntVento > 0 Then
                                quadr = Round(dirVento / 45#)
                                If quadr = 0 Then quadr = 8
                                direzione(quadr) = direzione(quadr) + 1
                                intensita(quadr) = intensita(quadr) + IntVento
                            End If
                        Next ora

                        If cont > 0 Then
                            .windIntensity = sum / cont
                        Else
                            .windIntensity = NO_DATA
                        End If

                        If computeDailyExtremes Then .windIntensityMax = ValMax

                        'cerca direzione prevalente
                        dirPre = 0
                        For quadr = 1 To 8
                            If direzione(quadr) > 0 Then
                                If dirPre = 0 Then
                                    dirPre = quadr
                                ElseIf (direzione(quadr) > direzione(dirPre)) _
                                    Or ((direzione(quadr) = direzione(dirPre)) _
                                    And (intensita(quadr) > intensita(dirPre))) Then
                                    dirPre = quadr
                                End If
                            End If
                        Next quadr

                        .windDirection = IIf(dirPre = 0, Definitions.NO_DATA, dirPre * 45)
                    End If
                    ...
                    If computeRad Then
                        ...
                        For ora = 1 To 24
                            radh = MeteoGrid.Point(myRow, myCol).values_H.rad_h(ora)
                            If radh <> NO_DATA Then
                                sum = sum + radh * Conversion.RADIATION_WH_MJ
                                cont = cont + 1
                            End If
                        Next ora

                        .Rad = IIf(cont > 0, sum, Definitions.NO_DATA)
                    End If

                    'ETP PENMAN
                    If computeEt0Penman Then
                        sum = 0: cont = 0
                        For ora = 1 To 24
                            etph = MeteoGrid.Point(myRow, myCol).values_H.ET0_PM(ora)
                            If etph <> NO_DATA Then
                                sum = sum + etph
                                cont = cont + 1
                            End If
                        Next ora
                        .ET0_PM = IIf(cont > 0, sum, Definitions.NO_DATA)
                    End If
