...
void weatherGenerator2D::prepareWeatherGeneratorOutput()
{
    ...
    for (int iStation=0;iStation<nrStations;iStation++)
    {
        outputFileName = "outputDataC7/wgSimulation_station_" + QString::number(iStation) + ".txt";
        counter = 0;
    ...
    if(computeStatistics)
    {
        ...
        for (int iStation=0;iStation<nrStations;iStation++)
        {
            outputFileName = "outputDataC7/wgClimate_station_" + QString::number(iStation) + ".txt";
...
void weatherGenerator2D::precipitationCorrelationMatricesSimulation()
{
    ...
    FILE* fp;
    fp = fopen("outputDataC7/correlationMatrices.txt","w");
    for (int iMonth=0;iMonth<12;iMonth++)
    {
        fprintf(fp,"month %d \nsimulated - observed\n",iMonth+1);
        for (int i=0;i<nrStations;i++)
