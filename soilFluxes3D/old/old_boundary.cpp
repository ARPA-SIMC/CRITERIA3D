...
/*!
 * \brief soil surface resistance - Van De Griend and Owe (1994)
 * \param thetaTop
...
/*!
 * \brief soil surface resistance - Camillo and Gurney (1986)
 * \param theta
 * \param thetaSat
 * \return soil surface resistance (s m-1)
 */
 ...
 */
double computeAtmosphericSensibleFlux(long nodeIndex)
{
    if (nodeList[nodeIndex].boundary->Heat == nullptr || ! nodeList[nodeList[nodeIndex].up.index].isSurface)
        return 0;

    double myPressure = pressureFromAltitude(double(nodeList[nodeIndex].z));
    double myDeltaT = nodeList[nodeIndex].boundary->Heat->temperature - nodeList[nodeIndex].extra->Heat->T;
    double myCvAir = airVolumetricSpecificHeat(myPressure, nodeList[nodeIndex].boundary->Heat->temperature);

    return (myCvAir * myDeltaT * nodeList[nodeIndex].boundary->Heat->aerodynamicConductance);
}
 ...
 */
double computeAtmosphericLatentFlux(long nodeIndex)
{
    if (nodeList[nodeIndex].boundary->Heat == nullptr || ! nodeList[nodeList[nodeIndex].up.index].isSurface)
        return 0;

    double PressSat = saturationVaporPressure(nodeList[nodeIndex].boundary->Heat->temperature - ZEROCELSIUS);
    double ConcVapSat = vaporConcentrationFromPressure(PressSat, nodeList[nodeIndex].boundary->Heat->temperature);
 ...
 * \return vapor flux (kg m-2 s-1)
 */
double computeAtmosphericLatentFluxSurfaceWater(long nodeIndex)
{
    ...
    long downIndex = nodeList[nodeIndex].down.index;
    if (nodeList[downIndex].boundary->Heat == nullptr || nodeList[downIndex].boundary->type != BOUNDARY_HEAT_SURFACE)
        return 0.;


    // atmospheric vapor content (kg m-3)
    double PressSat = saturationVaporPressure(nodeList[downIndex].boundary->Heat->temperature - ZEROCELSIUS);
    double ConcVapSat = vaporConcentrationFromPressure(PressSat, nodeList[downIndex].boundary->Heat->temperature);
    double BoundaryVapor = ConcVapSat * (nodeList[downIndex].boundary->Heat->relativeHumidity / 100.);
 ...
 */
double computeAtmosphericLatentHeatFlux(long nodeIndex)
{
    if (nodeList[nodeIndex].boundary->Heat == nullptr || ! nodeList[nodeList[nodeIndex].up.index].isSurface)
        return 0;

    // J kg-1
    double lambda = latentHeatVaporization(nodeList[nodeIndex].extra->Heat->T - ZEROCELSIUS);
    // waterFlow: vapor sink source (m3 s-1)
    double latentHeatFlow = nodeList[nodeIndex].boundary->waterFlow * WATER_DENSITY * lambda;
...
// [-]
double getSurfaceWaterFraction(int nodeIndex)
{
    if (! nodeList[nodeIndex].isSurface)
        return 0.0;

    double h = std::max(nodeList[nodeIndex].H - nodeList[nodeIndex].z, 0.); // [m]
    double h0 = std::max(double(nodeList[nodeIndex].pond), 0.001);          // [m]
...
void updateBoundaryWater (double deltaT)
{
    for (long i = 0; i < myStructure.nrNodes; i++)
    {
        ...
        switch (nodeList[i].boundary->type)
        {
            case BOUNDARY_RUNOFF:
                double avgH, hs, maxFlow, v, flow;
                avgH = (nodeList[i].H + nodeList[i].oldH) * 0.5;        // [m]

                // Surface water available for runoff [m]
                hs = std::max(avgH - (nodeList[i].z + nodeList[i].pond), 0.);
                if (hs < EPSILON_RUNOFF)
                    break;

                maxFlow = (hs * nodeList[i].volume_area) / deltaT;      // [m3 s-1] maximum flow available during the time step
                // Manning equation
                v = pow(hs, 2./3.) * sqrt(nodeList[i].boundary->slope) / nodeList[i].Soil->roughness;
                // on the surface boundaryArea is a side [m]
                flow = nodeList[i].boundary->boundaryArea * hs * v;     // [m3 s-1]
                nodeList[i].boundary->waterFlow = -std::min(flow, maxFlow);

                break;
            case BOUNDARY_FREEDRAINAGE:
                // Darcy unit gradient -> dH=dz=L -> dH/L=1
                nodeList[i].boundary->waterFlow = - nodeList[i].k * nodeList[i].up.area;
                break;
            case BOUNDARY_FREELATERALDRAINAGE:
                // Darcy gradient = slope -> dH=dz slope=dz/L -> dH/L=slope
                ...
                break;
            case BOUNDARY_PRESCRIBEDTOTALPOTENTIAL:
                double L, boundaryZ, boundaryK, meanK, dH;
                // water table
                L = 1.0;                             // [m]
                boundaryZ = nodeList[i].z - L;       // [m]
                //boundaryK;                           // [m s-1]
                ...
                meanK = computeMean(nodeList[i].k, boundaryK);
                dH = nodeList[i].boundary->prescribedTotalPotential - nodeList[i].H;
                nodeList[i].boundary->waterFlow = meanK * nodeList[i].boundary->boundaryArea * (dH / L);
                break;
            case BOUNDARY_HEAT_SURFACE:
                if (!myStructure.computeHeat || !myStructure.computeHeatVapor)
                    break;

                long upIndex; double surfaceWaterFraction, evapFromSoil;
                upIndex = NOLINK;
                ...
                evapFromSoil = computeAtmosphericLatentFlux(i) / WATER_DENSITY * nodeList[i].up.area;
    ...
    for (long i = 0; i < myStructure.nrNodes; i++)
    {
        ...
        switch (nodeList[i].boundary->type)
        {
            case BOUNDARY_HEAT_SURFACE:
                ...
                // TODO [K] ?
                currentCourant = fabs(nodeList[i].extra->Heat->Qh) * timeStep / (heatCapacity * nodeList[i].volume_area);
            ...
            case BOUNDARY_PRESCRIBEDTOTALPOTENTIAL:
                if (myStructure.computeWater && myStructure.computeHeatAdvection)
                {
                    ...
                    double advTemperature = (waterFlux < 0) ? nodeList[i].extra->Heat->T : nodeList[i].boundary->Heat->fixedTemperature;
