...
namespace gis
{
    ...
    {
        Crit3DGeoPoint geoPoint[2];
        Crit3DUtmPoint utmPoint[2];

        getLatLonFromRowCol(latLonHeader, latLonWindow.v[0], &(geoPoint[0]));
        getUtmFromLatLon(utmZone, geoPoint[0], &(utmPoint[0]));
        getRowColFromXY(utmHeader, utmPoint[0], &(utmWindow->v[0]));

        getLatLonFromRowCol(latLonHeader, latLonWindow.v[1], &(geoPoint[1]));
        getUtmFromLatLon(utmZone, geoPoint[1], &(utmPoint[1]));
