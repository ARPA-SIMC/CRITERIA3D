...
namespace soilFluxes3D::v1
{
    ...
    */
                            ...
                            int maxApproximationsNumber, int ResidualTolerance, double MBRThreshold)
    {
        ...
        if (ResidualTolerance < 5)
            ResidualTolerance = 5;
        if (ResidualTolerance > 16)
            ResidualTolerance = 16;
    ...
    */
                        ...
                        bool isBoundary, int boundaryType, float slope, float boundaryArea)
    {
        ...
        if ((myIndex < 0) || (myIndex >= myStructure.nrNodes))
            return(INDEX_ERROR);
    ...
    int DLL_EXPORT __STDCALL setNodeLink(long nodeIndex, long linkIndex, short direction, float interfaceArea)
    {
        /*! error check */
        if (nodeList == nullptr)
            return MEMORY_ERROR;

        if ((nodeIndex < 0) || (nodeIndex >= myStructure.nrNodes) || (linkIndex < 0) || (linkIndex >= myStructure.nrNodes))
            return INDEX_ERROR;
        ...
        {
            case UP:
                nodeList[nodeIndex].up.index = linkIndex;
                nodeList[nodeIndex].up.area = interfaceArea;
                nodeList[nodeIndex].up.sumFlow = 0;

                if (myStructure.computeHeat || myStructure.computeSolutes)
                {
                    nodeList[nodeIndex].up.linkedExtra = new(TCrit3DLinkedNodeExtra);
                    initializeLinkExtra(nodeList[nodeIndex].up.linkedExtra, myStructure.computeHeat, myStructure.computeSolutes);
                }

                break;
            case DOWN:
                nodeList[nodeIndex].down.index = linkIndex;
                nodeList[nodeIndex].down.area = interfaceArea;
                nodeList[nodeIndex].down.sumFlow = 0;

                if (myStructure.computeHeat || myStructure.computeSolutes)
                {
                    nodeList[nodeIndex].down.linkedExtra = new(TCrit3DLinkedNodeExtra);
                    initializeLinkExtra(nodeList[nodeIndex].down.linkedExtra, myStructure.computeHeat, myStructure.computeSolutes);
                }

                break;
            case LATERAL:
                j = 0;
                while ((j < myStructure.nrLateralLinks) && (nodeList[nodeIndex].lateral[j].index != NOLINK))
                    j++;

                if (j == myStructure.nrLateralLinks)
                    return (TOPOGRAPHY_ERROR);

                nodeList[nodeIndex].lateral[j].index = linkIndex;
                nodeList[nodeIndex].lateral[j].area = interfaceArea;
                nodeList[nodeIndex].lateral[j].sumFlow = 0;

                if (myStructure.computeHeat || myStructure.computeSolutes)
                {
                    nodeList[nodeIndex].lateral[j].linkedExtra = new(TCrit3DLinkedNodeExtra);
                    initializeLinkExtra(nodeList[nodeIndex].lateral[j].linkedExtra, myStructure.computeHeat, myStructure.computeSolutes);
                }
    ...
    */
    double DLL_EXPORT __STDCALL getWaterContent(long nodeIndex)
    {
        ...
        return theta_from_Se(unsigned(nodeIndex));
    }
    ...
    double DLL_EXPORT __STDCALL getSumLateralWaterFlowIn(long nodeIndex)
    {
        ...
        double sumLateralFlowIn = 0.0;
        for (short i = 0; i < myStructure.nrLateralLinks; i++)
            if (nodeList[nodeIndex].lateral[i].index != NOLINK)
                if (nodeList[nodeIndex].lateral[i].sumFlow > 0)
                    sumLateralFlowIn += nodeList[nodeIndex].lateral[i].sumFlow;

        return sumLateralFlowIn;
    }
    ...
    double DLL_EXPORT __STDCALL getSumLateralWaterFlowOut(long nodeIndex)
    {
        ...
        double sumLateralFlowOut = 0.0;
        for (short i = 0; i < myStructure.nrLateralLinks; i++)
            if (nodeList[nodeIndex].lateral[i].index != NOLINK)
                if (nodeList[nodeIndex].lateral[i].sumFlow < 0)
                    sumLateralFlowOut += nodeList[nodeIndex].lateral[i].sumFlow;

        return sumLateralFlowOut;
    }
    ...
    */
    ...
    int DLL_EXPORT __STDCALL setFixedTemperature(long nodeIndex, double myT, double myDepth)
    {
        ...
        if (nodeList[nodeIndex].boundary == nullptr)
            return(BOUNDARY_ERROR);
        if (nodeList[nodeIndex].boundary->Heat == nullptr)
            return(BOUNDARY_ERROR);
        if (nodeList[nodeIndex].boundary->type != BOUNDARY_PRESCRIBEDTOTALPOTENTIAL && nodeList[nodeIndex].boundary->type != BOUNDARY_FREEDRAINAGE)
            return(BOUNDARY_ERROR);
    ...
    int DLL_EXPORT __STDCALL setHeatBoundaryWindSpeed(long nodeIndex, double myWindSpeed)
    {
        ...
        if ((nodeIndex < 0) || (nodeIndex >= myStructure.nrNodes))
            return(INDEX_ERROR);
        if ((myWindSpeed < 0) || (myWindSpeed > 1000))
            return(PARAMETER_ERROR);
    ...
    */
    double DLL_EXPORT __STDCALL getTemperature(long nodeIndex)
    {
        if (nodeList == nullptr)
            return(TOPOGRAPHY_ERROR);
        if (nodeIndex >= myStructure.nrNodes)
            return(INDEX_ERROR);
        if (!isHeatNode(nodeIndex))
            return(MEMORY_ERROR);
    ...
    */
    double DLL_EXPORT __STDCALL getBoundarySensibleFlux(long nodeIndex)
    {
        ...
        if (nodeList[nodeIndex].boundary == nullptr)
            return (INDEX_ERROR);
        if (nodeList[nodeIndex].boundary->type != BOUNDARY_HEAT_SURFACE)
            return (INDEX_ERROR);
    ...
    */
    double DLL_EXPORT __STDCALL getBoundaryLatentFlux(long nodeIndex)
    {
        ...
        if (nodeIndex >= myStructure.nrNodes)
            return (INDEX_ERROR);
        if (!myStructure.computeHeat || !myStructure.computeWater || !myStructure.computeHeatVapor)
            return (MISSING_DATA_ERROR);
        if (nodeList[nodeIndex].boundary == nullptr)
            return (INDEX_ERROR);
        if (nodeList[nodeIndex].boundary->type != BOUNDARY_HEAT_SURFACE)
    ...
    */
    double DLL_EXPORT __STDCALL getNodeVapor(long nodeIndex)
    {
        if (nodeList == nullptr)
            return(TOPOGRAPHY_ERROR);
        if (nodeIndex >= myStructure.nrNodes)
        ...
        double h = nodeList[nodeIndex].H - nodeList[nodeIndex].z;
        double T = nodeList[nodeIndex].extra->Heat->T;
    ...
    */
    double DLL_EXPORT __STDCALL getHeat(long nodeIndex, double h)
    {
        if (nodeList == nullptr)
            return(TOPOGRAPHY_ERROR);
        if (nodeIndex >= myStructure.nrNodes)
        ...
        if (! myStructure.computeHeat)
            return(MISSING_DATA_ERROR);
        if (nodeList[nodeIndex].extra->Heat == nullptr)
            return MISSING_DATA_ERROR;
        if (nodeList[nodeIndex].extra->Heat->T == NODATA)
            return MISSING_DATA_ERROR;

        double myHeat = SoilHeatCapacity(nodeIndex, h, nodeList[nodeIndex].extra->Heat->T) * nodeList[nodeIndex].volume_area  * nodeList[nodeIndex].extra->Heat->T;

        if (myStructure.computeWater && myStructure.computeHeatVapor)
        {
            double thetaV = VaporThetaV(h, nodeList[nodeIndex].extra->Heat->T, nodeIndex);
            myHeat += thetaV * latentHeatVaporization(nodeList[nodeIndex].extra->Heat->T - ZEROCELSIUS) * WATER_DENSITY * nodeList[nodeIndex].volume_area;
        }
